 ...
 * 如果 TCB 布局变化，这里必须先编译失败，避免上下文切换读错字段。
 */
typedef char tcb_state_offset_must_be_24[(offsetof(tcb_t, state) == 24) ? 1 : -1];
typedef char tcb_attrs_offset_must_be_96[(offsetof(tcb_t, attrs) == 96) ? 1 : -1];
...
#define FLASH_SIZE  (2UL * 1024 * 1024)
#define SRAM_BASE   0x20000000UL
#define SRAM_SIZE   (384UL * 1024)
                            ...
                            uint32_t     stack_size)
{
    ...
    uint32_t flash_base = FLASH_BASE;
    uint32_t flash_size = FLASH_SIZE;
    uint32_t rasr_size  = mpu_calc_rasr_size(flash_size);

    tcb->mpu_regions[0][0] = flash_base | RBAR_VALID | 0;
    tcb->mpu_regions[0][1] = RASR_ENABLE
                            | AP_PRW_URO           /* 特权 RW, 用户 RO */
                            | ATTR_NORMAL_WBWA
                            | rasr_size;
                            /* XN=0: 用户可执行 */
    ...
    /* MPU Region 2: 用户栈 (RW, 底部子区域禁用 = 栈溢出守卫) */
    uint32_t eff_stack_size = tcb->stack_size;
    uint32_t stack_base = (uint32_t)tcb->stack_base;

    tcb->mpu_regions[2][0] = stack_base | RBAR_VALID | 2;
    tcb->mpu_regions[2][1] = mpu_stack_guard_rasr(stack_base, eff_stack_size, 0x01);
