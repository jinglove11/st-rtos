...
/* join 上限随迭代量自适应: 往返吞吐实测 ~11.6k/s,按 1/4 迭代数
 * (tick) 留 ~3 倍余量,30s 兜底 — 10k 次 (~1s) 与 1M 次 (~86s) 都覆盖。 */
#define SMP_JOIN_TIMEOUT_TICKS          ((SMP_STRESS_ITERATIONS / 4U) < 30000U ? 30000U : (SMP_STRESS_ITERATIONS / 4U))
...
TEST_K_MODULE(smp, test_smp_module);
